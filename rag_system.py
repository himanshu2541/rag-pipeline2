...
class RAGSystem:
    ...
    def __init__(self, config_instance=config):
        ...
        self.ingestor = Ingestor(self.embeddings, self.retriever_provider)

        # 4. Init chain provider (which needs the initial retriever)
        initial_retriever = self.retriever_provider.get_retriever()
        self.chain_provider = ChainProvider(self.llm, initial_retriever)
    ...
    def transcribe_audio(self, audio_file_path: str) -> str:
        """
        Transcribes audio to text without generating an answer.
        """
        logger.info(f"RAGSystem: Transcribing audio from {audio_file_path}")
        return self.stt_provider.transcribe(audio_file_path)
